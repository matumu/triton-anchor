...
static bool processPhiStruct(PHINode *phiNode) {
  ...
  IRBuilderBase::InsertPoint insertPoint = builder.saveIP();
  for (unsigned i = 0; i < structType->getNumElements(); ++i) {
    builder.SetInsertPoint(phiNode);
    PHINode *newPhi =
        builder.CreatePHI(structType->getElementType(i),
                          phiNode->getNumIncomingValues());
    for (unsigned j = 0; j < phiNode->getNumIncomingValues(); ++j) {
      BasicBlock *incomingBlock = phiNode->getIncomingBlock(j);
      builder.SetInsertPoint(incomingBlock->getTerminator());
      newPhi->addIncoming(
          builder.CreateExtractValue(phiNode->getIncomingValue(j), i),
          incomingBlock);
    }
...
PreservedAnalyses BreakStructPhiNodesPass::run(
    Function &function, FunctionAnalysisManager &) {
  bool changed = false;
