...
namespace {
struct UnsupportedLinearLayout {
  UnsupportedLinearLayout() {
    throw std::runtime_error(
        "LinearLayout belongs to the TritonGPU plugin and is not available "
        "in the hardware-independent triton-anchor core");
  }
};
} // namespace
...
void init_triton_anchor(nanobind::module_ &m);
...
NB_MODULE(libtriton, m) {
  ...
  auto anchor_m = m.def_submodule("anchor");
  init_triton_anchor(anchor_m);
  // Upstream Python imports these modules unconditionally.  Register only
  // import-compatible placeholders; their implementations live in OOT GPU
  // plugins and are deliberately not linked into the Anchor core.
  m.def_submodule("gluon_ir");
  auto linear_layout_m = m.def_submodule("linear_layout");
  py::class_<UnsupportedLinearLayout>(linear_layout_m, "LinearLayout")
      .def(py::init<>());
#ifdef TRITON_HAS_BACKENDS
