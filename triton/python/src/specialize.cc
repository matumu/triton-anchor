...
bool init_globals() noexcept try {
  ...
  // Hardware-specific Gluon descriptor types are registered by OOT backends.
  nvidia_tensor_descriptor_cls = nullptr;
  nvidia_tensor_descriptor_im2col_cls = nullptr;
  amd_tensor_descriptor_cls = nullptr;
...
std::pair<py::object, py::object> specialize_arg(PyObject *backend,
  ...
  if (nvidia_tensor_descriptor_cls &&
      PyObject_IsInstance(arg, nvidia_tensor_descriptor_cls)) {
    return handle_gluon_tensor_descriptor(backend, arg, is_const,
                                          specialize_value, align);
  }

  if (amd_tensor_descriptor_cls &&
      PyObject_IsInstance(arg, amd_tensor_descriptor_cls)) {
    return handle_gluon_tensor_descriptor(backend, arg, is_const,
                                          specialize_value, align);
  }
